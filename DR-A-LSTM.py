...
# E = Dense(100, activation=act)(input_lyr)
...
history = autoencoder.fit(TrainX, TrainX, epochs=500, batch_size=2048, 
                          validation_data=(TestX, TestX), verbose=0, callbacks=[callback])
...
tf.random.set_seed(seed)
np.random.seed(seed)
H = model.fit(Train, TrainL, epochs=500,batch_size=512, validation_data=(Test, TestL),callbacks=[EarlyStopping(monitor='val_accuracy', patience=50,restore_best_weights=True)],verbose=0, shuffle=True)
...
tf.random.set_seed(seed)
np.random.seed(seed)
H = model2.fit(TrainOne, TrainOneL, epochs=500,batch_size=512, validation_data=(TestOne, TestOneL),callbacks=[EarlyStopping(monitor='val_accuracy', patience=50,restore_best_weights=True)],verbose=0, shuffle=True)
...
final.fit(TrainOne,[TrainOne,TrainOneL], epochs=500, batch_size=2048, validation_data=(TestOne, [TestOne,TestOneL]),callbacks=callbacks, verbose=1, shuffle=True)
